...
> SEZER-AI-MVC Version: v1.5-test
> Deterministic canonical-matrix validation release based on real-project audit feedback.
...
The consistency check MUST explicitly evaluate `INV-001` through `INV-021`.
...
## v1.4 Canonical Capability Matrix and Severity Calibration

These rules are mandatory and override weaker wording elsewhere.

### 1. Single Canonical Capability Matrix

A Full Audit MUST maintain exactly one canonical capability matrix as the source of truth.

Every summary, counter, invariant check, executive summary, and `skil-rapor.md` section MUST derive its capability counts and statuses from this matrix.

Do not maintain multiple independently edited capability tables.

Required columns:

```markdown
| Capability ID | Capability | Required | Applicability | Execution | Assessment | Evidence Sufficiency | Limitation |
|---|---|---|---|---|---|---|---|
```

Each capability MUST have a stable `Capability ID`.
...
The exact list may expand when the repository requires additional capability rows, but duplicate rows for the same capability are forbidden.

### 2. Capability Cardinality Invariant
...
+ Failed
+ NotExecuted
+ NotApplicableRequiredRowsOnlyWhenSemanticallyAllowed
```
...
RequiredApplicableTotal
=
Completed + Partial + Failed + NotExecuted
```

because a capability marked `NOT_APPLICABLE` is normally excluded from "required applicable".

Therefore the report MUST also expose:

```text
RequiredCapabilitiesTotal
ApplicableRequiredCapabilities
NotApplicableRequiredCapabilities
```
...
Any mismatch is a contradiction.

If capability arithmetic does not balance:

```text
CrossReportConsistency != COMPLETED
OverallAuditStatus != FULL_AUDIT_COMPLETED
```

### 3. Canonical Status Invariant
...
all reports MUST derive status from the canonical matrix.
```

A capability MUST NOT be:

- `PARTIAL` in one section,
- `COMPLETED` in another,
- `NOT_EXECUTED` in another,

unless the text clearly refers to different sub-checks.

If sub-checks differ, represent them as separate capability/sub-capability IDs.

### 4. Applicability vs Execution
...
`NOT_APPLICABLE`
= the capability has no meaningful relevance to this repository.

`NOT_EXECUTED`
= the capability is relevant, but did not run.

Examples:

```text
Live DB audit in an EF application with inaccessible credentials:
...
If live database verification is impossible because credentials/access are absent, that is a limitation, not proof of non-applicability.

### 5. Database Truth-Level Matrix

Database truth MUST be represented as independent sub-levels:

```markdown
| Truth Level | Applicability | Execution | Evidence |
|---|---|---|---|
| Entity model | ... | ... | ... |
| EF configuration | ... | ... | ... |
| DbContext model | ... | ... | ... |
| Migrations | ... | ... | ... |
| Generated/expected schema | ... | ... | ... |
| Live database metadata | ... | ... | ... |
```

`CAP-DB-TRUTH` may be `COMPLETED` only if the skill's defined goal for that repository was fulfilled across all applicable truth levels.

If live DB is applicable but inaccessible, prefer:

```text
CAP-DB-TRUTH = PARTIAL
```

unless the capability is explicitly scoped to static database truth only.

### 6. Risk/Severity Calibration Gate

Every MEDIUM/HIGH/CRITICAL finding MUST pass a severity calibration step.

Required fields:

```text
Evidence class:
Exploitability / trigger condition:
Exposure:
Business impact:
Technical impact:
Preconditions:
...
Severity MUST be based on observed context, not category name alone.

### 7. Rate Limiting Rule

Absence of rate limiting is NOT automatically a HIGH severity issue.

Evaluate whether the affected endpoint is:

- internet-facing,
- unauthenticated,
- authentication-related,
- password/token-related,
- expensive to execute,
- state-changing,
- abuse-sensitive,
- externally callable at scale,
- protected by upstream gateway/WAF/CDN controls.

Possible classifications:

```text
Confirmed abuse exposure with meaningful impact:
MEDIUM/HIGH depending on evidence.

No rate limiting observed but abuse surface unverified:
POTENTIAL / verification-required.

Internal/admin-only low-volume endpoint:
LOW or informational.
```

A HIGH rate-limiting finding MUST document concrete abuse impact and exposure.

### 8. Missing Security Configuration Rule

Absence of explicit local configuration does not prove the control is absent at runtime.
...
check whether controls may exist through:
...
If those environments are not inspectable, use:

```text
UNKNOWN_REQUIRES_VERIFICATION
```

rather than confirmed absence where appropriate.

### 9. Severity Ceiling by Evidence Strength

Default severity ceilings:

```text
Evidence = UNKNOWN_REQUIRES_VERIFICATION
→ cannot be confirmed HIGH/CRITICAL

Evidence = POTENTIAL
→ normally LOW/MEDIUM unless strong impact evidence exists

Evidence = STRONGLY_INDICATED
→ severity may reach HIGH with justification

Evidence = CONFIRMED
→ full severity range permitted
```

If an exception is made, the report MUST explain why.

### 10. Self-Validation Counters

`skil-rapor.md` MUST include exactly one authoritative counter block:
...
ArithmeticBalanced: YES/NO
CanonicalMatrixContradictions: <count>
```

No second independently calculated counter block is allowed.

### 11. Completion Gate Update

`FULL_AUDIT_COMPLETED` requires:

- `ArithmeticBalanced = YES`
- `CanonicalMatrixContradictions = 0`
- INV-001 through INV-021 all PASS/NA
- no unresolved consistency exceptions
- all required applicable capabilities completed

`FULL_AUDIT_COMPLETED_WITH_LIMITATIONS` may be used when arithmetic is valid but some applicable capabilities are Partial/NotExecuted.

If arithmetic is invalid:

```text
FULL_AUDIT_PARTIAL
```

or fix the matrix before finalizing.

### 12. Invariant Table Expansion

The invariant table MUST now contain `INV-001` through `INV-021`.

Required additions:

```text
INV-014: Capability arithmetic balances.
INV-015: Canonical capability statuses are consistent everywhere.
```

### 13. Refactoring Recommendation Severity Independence

Refactoring priority and finding severity are related but not identical.

A MEDIUM finding may be high refactoring priority if it blocks many dependent fixes.

A HIGH finding does not automatically justify a heavyweight architectural pattern.

### 14. No Duplicate Architectural Defects

If two findings describe the same root cause, such as:

- direct DbContext injection in controllers,
- missing application boundary,

avoid double-counting the same issue across Architecture and Cross-Layer registers unless each finding captures materially different impact.
...
This reduces inflated finding counts.

### 15. Final Self-Check

Before writing the final response:

1. Recompute capability counts from the canonical matrix.
2. Verify arithmetic.
3. Verify all report status mentions map back to capability IDs.
4. Re-run INV-001 through INV-021.
5. Re-evaluate MEDIUM/HIGH/CRITICAL severities using the calibration gate.
6. Reclassify any unsupported rate-limiting/configuration claims.
7. Confirm DB truth-level status reflects unavailable live DB correctly.
8. Confirm no duplicate architectural root cause is counted twice without explanation.



## v1.5 Deterministic Canonical-Matrix Validator

These rules are mandatory and override weaker or conflicting self-validation wording elsewhere.

The purpose of v1.5 is to prevent the model from manually inventing capability totals, manually reconciling counters, or declaring invariants PASS without deriving them from the physical canonical matrix.

### 1. Canonical Matrix Is Data, Not Narrative

The canonical capability matrix is the sole authoritative dataset for capability status.
...
`Execution` MUST use the execution vocabulary defined by this skill.

`Assessment` MUST NOT be used as a substitute for Applicability or Execution.
...
Execution: NOT_EXECUTED
Assessment: NOT_APPLICABLE
Reason: repository contains no test project to execute
```
...
Execution: NOT_EXECUTED
Assessment: INCONCLUSIVE
Reason: database exists conceptually but credentials/access are unavailable
```

### 2. Physical Row Count Is Mandatory

Before producing counters, count the actual unique `Capability ID` rows in the canonical matrix.

Record:

```text
PhysicalCapabilityRows:
UniqueCapabilityIds:
DuplicateCapabilityIds:
```

These values MUST be derived from the matrix itself.

Do not estimate them from the expected capability list.

Do not copy them from an earlier summary.

### 3. Deterministic Counter Derivation

All counters MUST be derived from canonical matrix rows.

Definitions:

```text
RequiredCapabilitiesTotal
= count(rows where Required = YES)

NotApplicableRequiredCapabilities
= count(rows where Required = YES AND Applicability = NOT_APPLICABLE)

ApplicableRequiredCapabilities
= count(rows where Required = YES AND Applicability = APPLICABLE)

InconclusiveApplicabilityRequiredCapabilities
= count(rows where Required = YES AND Applicability = INCONCLUSIVE)
```

For required applicable rows only:

```text
Completed
= count(Required=YES AND Applicability=APPLICABLE AND Execution=COMPLETED)

Partial
= count(Required=YES AND Applicability=APPLICABLE AND Execution=PARTIAL)

Failed
= count(Required=YES AND Applicability=APPLICABLE AND Execution=FAILED)

NotExecuted
= count(Required=YES AND Applicability=APPLICABLE AND Execution=NOT_EXECUTED)
```

Never type these counters from memory.

### 4. Cardinality Equations

The following equations MUST be evaluated from the canonical rows:

```text
EQ-001:
PhysicalCapabilityRows = UniqueCapabilityIds
```

unless duplicate IDs exist, in which case validation fails.
...
If an additional valid execution state exists for an applicable required row, explicitly include that state in the equation and report it.

Do not set `ArithmeticBalanced: YES` unless all applicable equations pass.
...
INV-016:
PhysicalCapabilityRows = UniqueCapabilityIds
AND DuplicateCapabilityIds = 0
```

```text
INV-017:
All authoritative counters are reproducible directly from canonical matrix rows.
```

```text
INV-018:
Final response counters exactly match skil-rapor.md canonical-derived counters.
```

```text
INV-019:
Every canonical row contains the required columns:
Capability ID, Capability, Required, Applicability, Execution,
Assessment, Evidence Sufficiency, Limitation.
```

```text
INV-020:
Applicability, Execution, and Assessment are semantically non-conflicting.
```
...
is invalid unless the report explicitly explains a sub-check distinction.

### 6. Canonical-to-Summary Projection

The executive summary, final response, invariant table, and completion gate MUST be projections of the canonical matrix.

They are not independently authored sources.
...
Overall audit status
      ↓
Executive summary / final response
```

If the final response differs from `skil-rapor.md`, `INV-018 = FAIL`.

### 7. Canonical Matrix Freeze

Before generating the final response:

1. finish audit evidence collection,
2. finalize canonical matrix,
3. mark it `CANONICAL_MATRIX_FROZEN`,
4. derive counters,
5. evaluate equations,
6. evaluate invariants,
7. derive overall status,
8. generate summaries.

After freeze, do not manually change counters.

If a capability status changes, unfreeze the matrix, update the row, and recompute everything.

### 8. Deterministic Validator Artifact

When local scripting/tool execution is available, generate a small validator artifact under:
...
```json
{
  "physicalCapabilityRows": 0,
  "uniqueCapabilityIds": 0,
  "duplicateCapabilityIds": [],
  "requiredCapabilitiesTotal": 0,
  "applicableRequiredCapabilities": 0,
  "notApplicableRequiredCapabilities": 0,
  "inconclusiveApplicabilityRequiredCapabilities": 0,
  "completed": 0,
  "partial": 0,
  "failed": 0,
  "notExecuted": 0,
...
This artifact MUST be generated from parsed canonical rows, not manually composed to imitate validation.

If scripting is unavailable:

```text
ValidatorArtifact: NOT_EXECUTED
```

and the report must state that deterministic external validation was unavailable.

Do not falsely claim a validator ran.

### 9. Validator Script Guidance

If the environment permits safe READ_ONLY report tooling, a temporary/local script MAY parse `skil-rapor.md` after the canonical matrix is written.

The script may write only audit artifacts under `.sezer-audit/`.

It MUST NOT modify application source code.

Validation logic should:

- locate the canonical matrix,
- parse rows,
- normalize allowed enum values,
- reject missing Capability IDs,
- reject duplicate IDs,
- count physical rows,
- aggregate Required/Applicability/Execution,
- evaluate EQ-001 through EQ-003,
- emit JSON validation evidence.

Prefer deterministic parsing over LLM recounting.

### 10. Overall Status Must Follow Validator

If any of these occur:

- duplicate Capability IDs,
- missing required matrix columns,
- arithmetic mismatch,
- final-response counter mismatch,
- semantic Applicability/Execution conflict,
...
and unconditional `FULL_AUDIT_COMPLETED` is forbidden.

If the inconsistency can be repaired before finalization, repair the matrix and rerun validation.

### 11. Phase 0 Baseline Contract

Phase 0 MUST define its mandatory acceptance criteria before assigning status.
...
- dependency/architecture baseline,
- database/migration baseline,
- security baseline,
- performance baseline,
- rollback/change-safety baseline.
...
does not mean a test execution criterion was successfully completed.

It means:

```text
Test discovery: COMPLETED
Test execution: NOT_APPLICABLE or NOT_EXECUTED according to the defined criterion
Testing safety-net assessment: FINDINGS_PRESENT
```

Phase 0 may be `COMPLETED` only when every mandatory applicable acceptance criterion is satisfied or explicitly defined as not applicable before evaluation.

The phase acceptance table MUST be included in the refactoring plan.

### 12. Phase Status Cross-Report Invariant

Add:

```text
INV-021:
Every phase status in the final response exactly matches the authoritative phase acceptance table.
```

If `skil-rapor.md`, refactoring plan, executive summary, and final response disagree on Phase 0 or any phase:

```text
INV-021 = FAIL
```

### 13. Finding Root-Cause Deduplication

Add a canonical `Root Cause ID` when multiple findings originate from the same defect.

Example:

```text
ARCH-001: Direct DbContext usage in controllers
XLAYER-002: Cross-layer bypass caused by direct DbContext usage

Root Cause ID: RC-DBCTX-CONTROLLER-001
```

Reports may retain both perspectives, but the executive summary MUST NOT count them as two independent root causes.

### 14. Feature-Gap Evidence Rule

A placeholder implementation such as an empty ForgotPassword flow may be classified as:

```text
CONFIRMED_INCOMPLETE_IMPLEMENTATION
```

only when repository evidence proves that:

- the route/action/UI exists or promises the behavior, and
- the implementation does not perform the promised operation.

Do not infer missing product features from naming alone.

### 15. Silent Catch Severity Rule

Silent catch blocks are primarily reliability/observability findings unless evidence demonstrates security impact.
...
A silent catch MUST NOT be HIGH security severity solely because exceptions are suppressed.

Severity still depends on business/technical impact.

### 16. Architecture Confidence Calibration

Architecture confidence reflects confidence in the detected architecture, not architecture quality.
...
means evidence strongly supports the classification.

It does NOT mean the architecture scores 90/100.

Keep these concepts separate.

### 17. Final Authoritative Counter Block

`skil-rapor.md` MUST contain one and only one authoritative block:

```text
CANONICAL_MATRIX_FROZEN: YES
...
ArithmeticBalanced:
CanonicalMatrixContradictions:
ValidatorArtifact:
```

The final response MUST copy these values exactly.

### 18. Invariant Expansion

The mandatory invariant table is now:

```text
INV-001 → INV-021
```

No invariant may be marked PASS merely because the report claims there are no contradictions.

Each PASS must be traceable to canonical data, evidence, an equation result, or an authoritative table.

### 19. v1.5 Completion Requirements

`FULL_AUDIT_COMPLETED` requires all existing gates plus:

- canonical matrix frozen,
- EQ-001 PASS,
- EQ-002 PASS,
- EQ-003 PASS,
- INV-001 through INV-021 PASS/NA,
- no duplicate Capability IDs,
- no counter mismatch,
- no phase-status mismatch,
- deterministic validator artifact PASS when tooling was available.

`FULL_AUDIT_COMPLETED_WITH_LIMITATIONS` is allowed when audit evidence has legitimate limitations but canonical validation itself passes.

If canonical validation fails and cannot be repaired:
...
3. Evaluate INV-001 through INV-021.
