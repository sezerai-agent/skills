...
## Task Approval Workflow — Mandatory

After all audit reports are completed, create:

```text
<repository-root>/.sezer-audit/skill-task.md
```

`skill-task.md` must convert the verified audit findings and refactoring plan into actionable task headings.

For each task:

- create a clear task title,
- list the related finding IDs,
- add concrete sub-items describing what will be changed,
- list affected files/components when known,
- include the expected validation step,
- keep the task limited to one coherent scope.

Example:

```markdown
## TASK-001 — Authorization fixes

Related findings: SEC-001, MVC-004

- [ ] Add the required authorization policy to the affected controller/action.
- [ ] Verify server-side authorization for the related UI flow.
- [ ] Add or update relevant tests if available.
- [ ] Build and run applicable verification.

Status: WAITING_FOR_APPROVAL
```

After `skill-task.md` is created, STOP.

Do not implement any task automatically.

Before starting each task, explicitly ask the user for approval for that specific task.

Only after the user approves that task may the skill enter refactor/implementation mode for that task.

After completing and verifying the approved task:

1. update its status in `skill-task.md`,
2. report the result,
3. stop,
4. ask for approval before starting the next task.

Approval for one task does not authorize any other task.

This workflow overrides any weaker instruction that would allow multiple refactoring tasks to be implemented without separate user approval.
